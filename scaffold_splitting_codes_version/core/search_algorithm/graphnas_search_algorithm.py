...
import scipy.signal
import os
from core.estimation import Estimation
from core.search_algorithm import utils
...
class SearchController(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if args and args.softmax_temperature:
            self.softmax_temperature = float(args.softmax_temperature)
        else:
            self.softmax_temperature = softmax_temperature

        if args and args.tanh_c:
            self.tanh_c = float(args.tanh_c)
        else:
            self.tanh_c = tanh_c
...
class Search(object):
    ...
    def __init__(self,
        ...
        self.controller = SearchController(self.args,
        ...
        self.controller_optim = torch.optim.Adam(self.controller.parameters(),
                                                 lr=float(self.args.controller_lr))
    ...
    def search_operator(self):
        ...
        for self.epoch in range(self.start_epoch, int(self.args.controller_train_epoch)):
        ...
        self.search_based_on_trained_LSTM(int(self.args.search_scale))
        print("*" * 35, "use controller sampling ending", "*" * 35)
        search_total_time = time.time() - start_time

        path = os.path.split(os.path.realpath(__file__))[0][:-(17+len('core'))]+"logger"+'/'+str(self.args.data_name)+'/graphnas_logger'
        if not os.path.exists(path):
            os.makedirs(path)
        utils.experiment_time_save(path,
                                   self.args.data_name + "_search_total_time.txt",
                                   int(self.args.controller_train_epoch) +
                                   int(self.args.controller_train_parallel_num),
                                   int(self.args.search_scale),
                                   search_total_time)
    ...
    def get_reward(self, gnn_architecture_list, entropies_list):
        ...
        for val_auc in result:
            move_average_reward_list.append(self.move_average_reward_operator.get_reward(val_auc))

        rewards_list = []
        for index in range(len(move_average_reward_list)):
            reward_list = []
            reward_list.append(move_average_reward_list[index])
            rewards_list.append(reward_list + float(self.args.entropy_coeff) * entropies_list[index])
    ...
    def train_controller(self):
        ...
        for step in range(int(self.args.controller_train_parallel_num)):
        ...
        if self.baseline is None:
            ...
            decay = float(self.args.ema_baseline_decay)
        ...
        if float(self.args.controller_grad_clip) > 0:
            torch.nn.utils.clip_grad_norm(model.parameters(),
                                          float(self.args.controller_grad_clip))
    ...
    def search_based_on_trained_LSTM(self, search_scale):
        ...
        if not os.path.exists(path):
            os.makedirs(path)
        utils.experiment_data_save(path, self.args.data_name + ".txt", gnn_architecture_list, result)
