...
class ASAP_Pooling(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_weight=None, batch=None):
        ...
        # Add Self Loops
        fill_value = 1
        num_nodes = scatter_add(batch.new_ones(x.size(0)), batch, dim=0, dim_size=x.size(0))
        ...
        # NxF
        X_q, _ = scatter_max(x_pool_j, edge_index[0], dim=0, dim_size=x.size(0))
        # NxF
        M_q = self.lin_q(X_q)    
        # ExF
        ...
        #---Aggregation---
        # NxF
        out = scatter_add(v_j, edge_index[0], dim=0, dim_size=x.size(0))
