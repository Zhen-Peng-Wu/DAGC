...
# python fine_tune.py --data clintox --gpu 0 --num_blocks 8 --num_cells 1 --cell_mode full   --arch_filename exp_res/clintox-searched.txt   --cos_lr --BN --hyper_epoch 20
def get_args():
    ...
    #ft params
    parser.add_argument('--ft_mode', type=str, default='kfold', choices=['split_one', 'kfold'], help='data split function.')
...
def objective(args):
    ...
    vali_auc, test_auc, _, args = main(args)
    return {
        'loss': -vali_auc,
        'test_auc': test_auc,
        'test_std': _,
    ...
    for ind, l in enumerate(lines):
        try:
            ...
            for i in range(args1.std_times):
                # args.epochs = 100
                valid_auc, t_auc, _, test_args = main(args)
                print('cal std: times:{}, valid_Auc:{}, test_auc:{:.04f}'.format(i, valid_auc, t_auc))
                test_aucs.append(t_auc)
            test_aucs = np.array(test_aucs)

            # #re-evaluate the performance on the selected hypers
            # for i in range(args1.std_times):
            #     print('Train from scratch {}/{}: Test_acc:{:.04f}+-{:.04f}'.format(i, test_accs[i], test_stds[i]))
            print('test_results_times:{:.04f}+-{:.04f}'.format(np.mean(test_aucs), np.std(test_aucs)))
            res['aucs_train_from_scratch'] = test_aucs
            test_res.append(res)
