...
# python train_search.py --data clintox  --gpu 0 --num_blocks 8 --cell_mode full --num_cells 1 --agg gcn --cos_temp --BN
parser = argparse.ArgumentParser("")
parser.add_argument('--data', type=str, default='clintox', help='dataset name')
...
parser.add_argument('--num_blocks', type=int, default=8, help='framework layers')
...
parser.add_argument('--agg', type=str, default='gcn', help='aggregations used in this framework')
...
parser.add_argument('--data_fold', type=int, default=20, help='x_fold cross-validation')
...
def main(log_filename):
    ...
    model = Network(criterion, args.num_features, args.num_classes, hidden_size, dropout=args.dropout, args=args)
    model = model.cuda()
...
def train(data, model, criterion, model_optimizer, arch_optimizer):
        ...
        for i in range(args.w_update_epoch):
            ...
            if 'ogb' in args.data:
                ...
                for i in range(args.num_tasks):
                    is_valid = y[:, i] ** 2 > 0
                    error_loss += criterion[i](logits[is_valid, :, i], ((y[is_valid, i] + 1) / 2).long())
    ...
    if 'ogb' in args.data:
        ...
        return auc, total_loss / num_training_graphs
...
def infer(data_, model, criterion, test=False, single_path=False):
    ...
    for tmp_data in data:
        ...
        with torch.no_grad():
            ...
            y = tmp_data.y.view(-1, args.num_tasks)
            y_true.append(y)
            y_pred.append(logits)
    ...
    if 'ogb' in args.data:
        ...
        return total_loss/len(data.dataset), valid_auc
