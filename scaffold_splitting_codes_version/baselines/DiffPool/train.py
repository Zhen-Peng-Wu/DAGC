...
def evaluate(dataset, model, args, name='Validation', max_num_examples=None):
    ...
    for batch_idx, data in enumerate(dataset):
        ...
        y = data['label'].view(-1, args.num_tasks)

        y_true.append(y)
        y_scores.append(ypred)
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch_idx, data in enumerate(dataset):
            ...
            for i in range(args.num_tasks):
                is_valid = y[:, i] ** 2 > 0
                loss += model.loss(ypred[is_valid, :, i], ((y[is_valid, i] + 1) / 2).long().cuda())
        ...
        valid_auc = evaluate(val_dataset, model, args)
        test_auc = evaluate(test_dataset, model, args)
...
def benchmark_task_val(args, writer=None, feat='node-label'):
    ...
    for tr in range(test_repeat):
        ...
        nx_train_dataset = [to_networkx(d, to_undirected=True) for d in train_dataset]
        nx_valid_dataset = [to_networkx(d, to_undirected=True) for d in valid_dataset]
        nx_test_dataset = [to_networkx(d, to_undirected=True) for d in test_dataset]

        for i, pyg_data in enumerate(train_dataset):
            for j, feat in enumerate(pyg_data.x):
                nx_train_dataset[i].nodes[j]['feat'] = feat.numpy()
            nx_train_dataset[i].graph['label'] = pyg_data.y.numpy()
        for i, pyg_data in enumerate(valid_dataset):
            for j, feat in enumerate(pyg_data.x):
                nx_valid_dataset[i].nodes[j]['feat'] = feat.numpy()
            nx_valid_dataset[i].graph['label'] = pyg_data.y.numpy()
        for i, pyg_data in enumerate(test_dataset):
            for j, feat in enumerate(pyg_data.x):
                nx_test_dataset[i].nodes[j]['feat'] = feat.numpy()
            nx_test_dataset[i].graph['label'] = pyg_data.y.numpy()

        train_sampler = GraphSampler(nx_train_dataset, normalize=False, features=args.feature_type)
        valid_sampler = GraphSampler(nx_valid_dataset, normalize=False, features=args.feature_type)
        test_sampler = GraphSampler(nx_test_dataset, normalize=False, features=args.feature_type)

        train_loader = DataLoader(train_sampler, args.batch_size, shuffle=True, num_workers=args.num_workers)
        valid_loader = DataLoader(valid_sampler, args.batch_size, shuffle=False, num_workers=args.num_workers)
        test_loader = DataLoader(test_sampler, args.batch_size, shuffle=False, num_workers=args.num_workers)
        ...
        if args.method == 'soft-assign':
            ...
            print('Method: base')
            model = encoders.GcnEncoderGraph(
                    args.num_features, args.hidden_dim, args.output_dim, args.num_classes,
                    args.num_gc_layers, bn=args.bn, dropout=args.dropout, args=args).cuda()

        valid_auc, test_auc = train(train_loader, model, args, val_dataset=valid_loader, test_dataset=test_loader,
            writer=writer)
...
if __name__ == "__main__":
    ## the example of run command: python train.py --bmname clintox --cuda 0
    main()
