...
from dataset_utils.chem_splitters import scaffold_split
...
class MoleculeDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        if self.pre_filter is not None:
            index_list = [i for i, smile in enumerate(data_smiles_list) if self.pre_filter(smile)]
            data_list = [data_list[i] for i in index_list]
            data_smiles_list = [data_smiles_list[i] for i in index_list]

        if self.pre_transform is not None:
            data_list = [self.pre_transform(data, data_smiles_list[i]) for i, data in enumerate(data_list)]
