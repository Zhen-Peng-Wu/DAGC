...
## the example of run command: python fine_tune.py --data clintox  --arch_filename  ./exp_res/clintox-searched_XX.txt --hyper_epoch 20
def get_args():
    ...
    parser.add_argument('--ft_mode', type=str, default='kfold', choices=['split', 'kfold'], help='data split function.')
...
def run_fine_tune():
    ...
    for ind, l in enumerate(lines):
        try:
            ...
            #report the test acc with the best vali auc
            for d in trials.results:
                if -d['loss'] > c_vali_auc:
                    c_vali_auc = -d['loss']
                    c_test_auc = d['test_acc']
            ...
            for i in range(std_times):
                # args.epochs = 100
                vali_auc, t_auc, _, test_args = main(args)
                print('cal std: times:{}, valid_Auc:{}, test_auc:{:.04f}'.format(i, vali_auc, t_auc))
            ...
            res['accs_train_from_scratch'] = test_aucs
            test_res.append(res)

            logging.info('test_results of times:{:.04f}+-{:.04f}'.format(np.mean(test_aucs), np.std(test_aucs)))
