...
### the example of run command: bash run_DGCNN.sh DD 0
...
if [ ${fold} == 0 ]; then
  ...
  echo "Running 20-fold cross validation"
  start=`date +%s`
  #for i in $(seq 1 10)
  for i in $(seq 1 20)
  do
  ...
  tail -20 ${DATA}_acc_results.txt
  echo "Average accuracy and std are"
  tail -20 ${DATA}_acc_results.txt | awk '{ sum += $1; sum2 += $1*$1; n++ } END { if (n > 0) print sum / n; print sqrt(sum2 / n - (sum/n) * (sum/n)); }'
else
  CUDA_VISIBLE_DEVICES=${GPU} python main.py \
      -seed 1 \
      -data $DATA \
