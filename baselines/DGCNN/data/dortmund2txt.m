...
for ith_data = 1: size(datasets, 1)
    ...
    fold_size = floor(total / 20);
    p = randperm(total);
    % mkdir(dataset, '10fold_idx')
    mkdir(dataset, '20fold_idx')
    % for fold = 1 : 10
    for fold = 1 : 20
        ...
        fid = fopen(sprintf('%s/20fold_idx/test_idx-%d.txt', dataset, fold), 'w');
        ...
        fid = fopen(sprintf('%s/20fold_idx/train_idx-%d.txt', dataset, fold), 'w');
