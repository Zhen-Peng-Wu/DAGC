...
class Search(object):
    ...
    def search_operator(self):
        ...
        utils.experiment_time_save(path,
                                   self.gnn_parameter['data_name'] + "_search_total_time.txt",
    ...
    def get_reward(self, gnn_architecture_list, entropies_list):
        ...
        for gnn_architecture in gnn_architecture_list:
            tmp_res = Estimation(gnn_architecture=gnn_architecture,
                       data=self.data,
                       gnn_parameter=self.gnn_parameter).get_performance()
    ...
    def train_controller(self):
        ...
        if baseline is None:
            ...
            baseline = decay * baseline + (1 - decay) * rewards_list[max_index]
    ...
    def search_based_on_trained_LSTM(self, search_scale):
        ...
        if not os.path.exists(path):
            os.makedirs(path)
        utils.experiment_data_save(path, self.gnn_parameter['data_name']+ ".txt", gnn_architecture_list, result)
